...
### Fixed
- **Restored the trigger vocabulary the 3.3.0 description rewrite stripped out of 8 skills.** Rewriting descriptions to disambiguate siblings (audit H8/H10/M10/M11) optimised for *choosing between* skills and accidentally traded away the words that get a skill *selected at all* — `insights` no longer contained "insights", `intelligence` lacked "intelligence", `extract` lacked "extract", `profile` lacked "profile", and `anomalies` had dropped **"Detect data anomalies"** entirely, keeping "anomal-" only as a cross-reference to its sibling. With the eight agents removed, `anomaly-detector`'s description was the *other* carrier of that phrasing, so "detect anomalies" / "run anomaly detection" had no strong match left anywhere in the plugin. Each of the 8 now leads with its own identity vocabulary and keeps the scope clause that disambiguates it from siblings (`dashboard` one-month vs `insights` full-year vs `intelligence` workbook vs `extract` raw export; `profile` whole-history statistics vs `anomalies` fiscal-year severity ranking).
- **Fixed invalid YAML frontmatter in 5 skills — an unquoted `: ` inside the `description` scalar.** `dashboard`, `extract`, `insights` and `profile` would have shipped broken by the rewrite above; **`org-context__internal` has been broken since 3.3.0** (`show org-wide context: users, …`). A strict YAML parse fails with *"mapping values are not allowed here"*, so a strict loader would fail to register the skill at all — the lenient split-on-first-colon parse that Claude Code appears to use is why this went unnoticed. All five rephrased to use an em dash; every one of the plugin's 26 skills now parses as valid YAML.

### Verified (no change required)
- **Removing the 8 agents caused no capability regression** — reviewed each deleted agent against its surviving skill(s). Every substantive behaviour is already covered: `forecast`'s variance-magnitude bands (`<5%` / `5-10%` / `10%+`) are defined in `forecast-variance`; `anomaly-detector`'s large-table runtime expectations are in `anomalies-report`; `finance-analyst` was a composition of `tables` + `profile` + `query` + `anomalies` rather than a distinct capability. Note this corrects the duplication audit (and the deletion commit that repeated it), which claimed `forecast-variance` did *not* define those bands — it does. What the deletion does remove is the autonomous **delegation path** (isolated-context subagent runs), which was the intent.
