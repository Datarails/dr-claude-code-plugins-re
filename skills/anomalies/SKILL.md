...
profile/anomaly tools are deliberately thin — they return baseline
...
|---|---|---|
| `detect_anomalies` | `MIN, MAX, AVG, COUNT` for the first 5 numeric fields | outliers, std dev, percentiles, severity, temporal gaps, duplicates, referential integrity |
...
| `aggregate_table_data` | grouped totals with no row limit | anything not expressible as GROUP BY + aggregation |

For severity, percentiles, std dev, z-scores, and most named anomaly
categories below — the skill computes them, not the tool.
...
4. For each candidate-key or category field: `aggregate_table_data`
   with that field as the dimension and `COUNT` as the metric. This is
   the only way to get per-value frequencies and null counts.
...
- Approximate spread = `(MAX - MIN)`; flag values outside
  ...
  available).
- To surface specific rows: call `aggregate_table_data` with the
  numeric field bucketed into ranges (e.g. group by SIGN + decade
  bucket) and the rest as dimensions. **Do not** try
  `get_records_by_filter` with `>` / `<` — those are rejected by
  the backend.
- Severity heuristic: ≥100 rows or ≥1% of total outside the band →
  HIGH; ≥1000 rows or ≥10% → CRITICAL; otherwise MEDIUM.

**Missing values**
- From the `aggregate_table_data` GROUP BY result, the null/blank
  group shows up as an unnamed bucket. Sum its `COUNT` and divide by
  total rows for the null rate.
- Severity heuristic: ≥10% null on a non-nullable field → CRITICAL;
  ≥1% → HIGH; <1% → LOW.

**Duplicates**
- Pick a candidate composite key (e.g. `[transaction_id]` or
  `[amount, vendor, posting_date]`). Call `aggregate_table_data`
  grouping by that key with `COUNT`. Client-side, filter groups where
  `COUNT > 1`.
...
- Date columns are stored as epoch integers and the filter API
  rejects them entirely. Use `aggregate_table_data` with the date
  field in `dimensions` and inspect the returned buckets for values
  beyond today or before a plausible earliest date.
...
- Per-character data hygiene (trailing whitespace, casing) —
  requires raw-row scanning, which is capped at 500 rows.
...
| `missing` | Null bucket from `aggregate_table_data` GROUP BY |
| `duplicates` | `aggregate_table_data` GROUP BY candidate key + filter COUNT > 1 |
| `rare-category` | `aggregate_table_data` GROUP BY field + filter COUNT < threshold |
| `temporal` | Aggregate by date dimension + inspect for future/past-bound values |
...
something **the skill computed**, not something `detect_anomalies`
returned:
...
2. FUTURE-DATED TRANSACTIONS
   ...
   • Date columns cannot be filtered via the API — use the bucket
     list above to identify the affected rows.
...
2. **Surface specific rows** — for outliers/duplicates, use
   `aggregate_table_data` with bucketed dimensions to get IDs, then
   pass the IDs to `/dr-query` as an `in` list.
...
- `/dr-tables` — schema discovery + `aggregate_table_data` source
