...
First, read the series correctly: every returned row is a real month —
responses append **no total row** (preamble item 4), so the series total
is your own sum of the rows. A `[null]` date bucket is unattributed
