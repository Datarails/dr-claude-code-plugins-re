...
(data-scope preamble, item 4): every row in an aggregation response is a
real category and **no total row is appended to the rows**. **Total Expenses
reads from the response's top-level `totals` field** — the Step 3/4 calls are
already filtered to the discovered expense buckets, so their `totals` IS total
expenses, exact even when the rows are truncated (`totals` absent + complete
rows: sum them; absent + truncated: re-run the aggregation). Per-category
amounts, shares of that total, rankings, and bucket counts derive from the
rows — complete responses only.
Null groups
